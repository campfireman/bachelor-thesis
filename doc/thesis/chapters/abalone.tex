...
\section{Board and move representations}
Cube coordinates (cf. figure \ref{abalone_coordinate_systems} (b)) as proposed by Patel \cite{noauthor_red_nodate} are a convenient way to represent hexagonal boards as they allow for simpler application of a wide variety of formulas and algorithms like Manhattan distance, accessing neighbors, finding paths etc.
\begin{figure}[!h]
    \centering
...
\begin{figure}[!h]
    ...
    \label{input_matrix}
\end{figure}
A string-based format was introduced to serialize the moves more easily. The regex for the format is the following:
