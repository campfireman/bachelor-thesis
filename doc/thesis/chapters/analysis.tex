...
$$
    f(x) = mx + b,
$$
with $ x, m,b \in \mathbb{R} $. We could use this function to make predicions about one dimensional input data. For example we might have living space of houses in $ m^2 $ $ X_i $ and their corresponding price $ Y_i $. We could choose the bias $ b $ and the scale $ m $ such that the squared error for the given data is minimal. When given new data our linear model can make predictions of the potential prices of the houses.


\begin{figure}
    \centering
    \includegraphics[height=7cm, keepaspectratio]{neural_networks.png}
    \caption{A fundatmental shift in how we think about programming}
    \label{neural_network}
\end{figure}

\paragraph{Activation function}
Such linear functions are one type of \textit{neuron} that might be utilized in a neural network\ref{neural_network}.

We might arrange multiple of these functions either in a chain, parallel or in layers as depticted in figure
